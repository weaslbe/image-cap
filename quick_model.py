from keras.layers import Conv2D, Input, Concatenate, Dense, Dropout, TimeDistributed, RepeatVector, Embedding, LSTM
...
class QuickImageCaptioningModel:
    ...
    def build_image_model(self, coco_image):
        conv_blocks = []
        conv_1 = Conv2D(100, (3, 3), activation='relu', data_format='channel_last')(coco_image)
        conv_2 = Conv2D(100, (5, 5), activation='relu', data_format='channel_last')(coco_image)
        conv_3 = Conv2D(100, (7, 7), activation='relu', data_format='channel_last')(coco_image)
    ...
    def language_model(self, seq_length,
        ...
        lstm = LSTM(128, return_sequence=True)(lstm_in)

        predictions = TimeDistributed(Dense(dict_length,
                                            activation='softmax'),
                                      name='out')(lstm)
    ...
    def build_model(self, dict_length):
        seqlen = 20
        coco_image = Input(shape=(400, 600))

        img_emb = self.build_image_model(coco_image)

        prev_words = Input(shape=(seqlen), name='prev_words')

        lang_model = self.lang_model(seqlen, dict_length, prev_words)
